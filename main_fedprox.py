...
for epoch in range(num_epochs):  # loop over the dataset multiple times
    ...
    for zipdata in zip(*subtrainloader):
        ...
        h = h - 1
        if h == 0:
            parallel_apply_FedProx(net_w, zipdata_list, criterion, weights, lr, mu, devices)
...
np.savetxt(path+'loss_fedprox_2020_%d_%.2f.txt' % (H, mu), np.array(loss))
np.savetxt(path+'comm_fedprox_2020_%d_%.2f.txt' % (H, mu), np.array(comm))
np.savetxt(path+'testacc_fedprox_2020_%d_%.2f.txt' % (H, mu), np.array(test_acc))
